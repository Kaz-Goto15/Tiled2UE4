...
bool Parser::Init(wchar_t* exePath) {
	...
	if (filesystem::exists(lpPath)) {
		...
		try {
			...
			OutText(L"リンクパスファイルの読込時に問題が発生しました：" + eWStr, OS_ERROR);
			return false;
		}
	}
	else {
...
bool Parser::Read(wstring _path, json* _data)
{
	...
	//else {
		...
		try { *_data = json::parse(f); }
			...
			wstring eWStr = GetWC(e.what());
			OutText(L"マップデータの読込に失敗しました：" + eWStr, OS_ERROR);
			return false;
		}
		...
		if (!isLinked) {
			...
			while (!endFlag) {
				...
				case '2':	//エクスプローラ選択
					...
					if (exprWStr == L"") {
						OutText(L"ファイルが選択されませんでした。", OS_INFO);
						break;
					}
...
void Parser::Parse(wstring _path, json _data)
{
	...
	{"version", _data["version"]}
		...
		wstring eWStr = GetWC(e.what());
		OutText(L"保存時にエラーが発生しました(" + eWStr + L")", OS_ERROR);
	}
}
...
void Parser::StoreWStr(wstring* wstr, json* j)
{
	...
	try {
		*wstr = converter.from_bytes(j->get<string>());
		...
		wstring eWStr = GetWC(e.what());
		OutText(L"jsonに格納されたデータを文字列として認識できませんでした(" + eWStr + L")", OS_ERROR);
	}
...
string Parser::ConvStr(wstring& src)
{
	...
	//UTF-8として認識してないのか、対応してないのか　読込はできんのに？
}
